...
from src.models import Classifier, predict_with_confidence, train_cylinder_condition_model
...
def train_demo_model(seed: int = 42) -> tuple[Classifier, dict]:
    x, y, baseline = create_demo_training_data(seed)
    model = train_cylinder_condition_model(x, y, seed=seed)
    return model, baseline


def analyze_once(fs: float, duration: float, operating_hours: float,
                 model: Classifier, baseline: dict,
    ...
    raw_prediction, predicted_probability = predict_with_confidence(model, model_values)
