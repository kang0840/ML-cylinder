...
import numpy as np
from src.models import Classifier, predict_with_confidence, train_simulated_asset_model
...
def _fit_conveyor_model(seed: int = 42) -> Classifier:
    ranges = ((0.0, 0.32, "NORMAL"), (0.38, 0.68, "WARNING"), (0.75, 1.0, "ERROR"))
    def measurement(severity: float, rng: np.random.Generator) -> dict[str, float]:
        return _conveyor_measurement(severity, float(rng.uniform(17.0, 25.0)), rng)
    return train_simulated_asset_model(
        measurement, CONVEYOR_FEATURES, ranges, samples_per_class=450,
        seed=seed, asset="conveyor",
    )


def _fit_cylinder_model(seed: int = 84) -> Classifier:
    ranges = ((0.0, 0.22, "A"), (0.27, 0.48, "B"), (0.53, 0.74, "C"), (0.8, 1.0, "D"))
    def measurement(severity: float, rng: np.random.Generator) -> dict[str, float]:
        return _cylinder_measurement(severity, float(rng.uniform(240.0, 420.0)), rng)
    return train_simulated_asset_model(
        measurement, CYLINDER_FEATURES, ranges, samples_per_class=400,
        seed=seed, asset="cylinder",
    )
...
class FactoryDigitalTwin:
    ...
    def read_conveyor(self, equipment_id: str) -> dict[str, object]:
        with self.lock:
            ...
            prediction, confidence = predict_with_confidence(
                self.conveyor_model, [sensor[name] for name in CONVEYOR_FEATURES],
            )
    ...
    def read_cylinder(self, equipment_id: str) -> dict[str, object]:
        with self.lock:
            ...
            prediction, confidence = predict_with_confidence(
                self.cylinder_model, [sensor[name] for name in CYLINDER_FEATURES],
            )
            zone = str(prediction)
