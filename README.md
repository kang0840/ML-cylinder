...
| API | 설명 |
|-----|------|
| `GET /api/conveyor` | CV-01·CV-02의 연속 진동, 회전 주파수, 고조파와 음향 대역 특징을 제공합니다. |
| `GET /api/cylinder` | CY-01·CY-02의 왕복 충격, 스트로크 시간, 진동과 누설음 대역 특징을 제공합니다. |
...
컨베이어와 실린더는 ADXL345와 INMP441을 공통으로 사용하지만 분석 구간과 특징, 모델을 공유하지 않습니다. 컨베이어 모델은 연속 신호에서 회전 주파수, 고조파와 베어링 이상음을 분석합니다. 실린더 모델은 동작 이벤트별 충격 에너지, 스트로크 시간과 누설음 대역을 분석합니다. 전류, 온도와 압력처럼 두 센서로 직접 측정할 수 없는 값은 사용하지 않습니다. 음압은 별도 마이크 보정 없이 계산 가능한 dBFS로 표현합니다.
