...
- 컨베이어와 공압 실린더의 독립 더미 센서 및 AI 상태 모델 제공
...
기존 Flask 서버는 컨베이어와 실린더 데이터를 서로 다른 API로 제공합니다.

| API | 설명 |
|-----|------|
| `GET /api/conveyor` | CV-01·CV-02의 모터 속도, 전류, 온도, 진동, 벨트 슬립과 AI 상태를 제공합니다. |
| `GET /api/cylinder` | CY-01·CY-02의 압력, 진동, 충격, 소음, 주요 주파수와 AI Zone을 제공합니다. |
| `GET /api/history` | 설비별 시계열 측정 이력을 제공합니다. |
...
컨베이어와 실린더는 동일한 데이터나 모델을 공유하지 않습니다. 컨베이어 모델은 모터 부하, 베어링 마모와 벨트 슬립에 따른 전류·온도·진동 변화를 학습하고, 실린더 모델은 공압 누설, 실 마찰과 종단 충격에 따른 압력·진동·소음 변화를 별도로 학습합니다.
